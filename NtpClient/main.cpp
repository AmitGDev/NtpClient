...
std::chrono::sys_seconds ToUnixEpoch(
  ...
  const auto kNtpToUnixOffset = (kDaysIn70Years + kLeapDays) * kSecondsInDay;

  const auto unix_seconds =
      std::chrono::seconds{static_cast<long long>(ntp_timestamp.seconds)} -
      kNtpToUnixOffset;
...
int main() {
  ...
  if (!invalid_result) {
    std::cout << "Expected error occurred: " << invalid_result.error().message()
              << "\n";
  }
}